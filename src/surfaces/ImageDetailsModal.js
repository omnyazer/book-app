...
import { saveImage, unsaveImage } from '../store/savedImages';
...
export default function ImageDetailsModal({ navigation, route }) {
  ...
  const isSaved = useSelector((state) =>
    state.savedImages.some((it) => String(it.id) === String(payload.id))
  );
  ...
  const toggleSave = () => {
    if (isSaved) dispatch(unsaveImage(payload.id));
    else dispatch(saveImage(payload));
  };
